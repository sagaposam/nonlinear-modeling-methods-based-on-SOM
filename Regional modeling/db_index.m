...
% DB_INDEX Davies-Bouldin clustering evaluation index.^^DB聚类评价指标
%
% [t,r] = db_index(D, cl, C, p, q)
%
%  Input and output arguments ([]'s are optional):  
%    D     (matrix) data (n x dim)
%          (struct) map or data struct^^映射或者数据结构
%    cl    (vector) cluster numbers corresponding to data samples (n x
%                    1)^^相对应于数据样本的聚类数量
%    [C]   (matrix) prototype vectors (c x dim) (default = cluster
%                   means)^^拓扑向量
%    [p]   (scalar) norm used in the computation (default == 2)^^计算中使用的范数
%    [q]   (scalar) moment used to calculate cluster dispersions (default = 2)
%                  ^^用于计算聚类离散度的矩
%    t     (scalar) Davies-Bouldin index for the clustering (=mean(r))
%    r     (vector) maximum DB index for each cluster (size c x 1)^^每个聚类的最大DB索引值   
% 
